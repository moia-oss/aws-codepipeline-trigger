...
exports.getInProgressProjectToBatchIds =
  exports.forwardLogEventsFromCodebuild =
  exports.getInProgressBuildId =
  exports.getCodebuildProjectsForPipeline =
    void 0;
...
const getCodeBuildsFromActions = (actions) => {
  return actions
    .map((action) => {
      var _a, _b;
      if (
        ((_a = action.actionTypeId) === null || _a === void 0 ? void 0 : _a.category) === 'Build' &&
        ((_b = action.actionTypeId) === null || _b === void 0 ? void 0 : _b.provider) === 'CodeBuild'
      ) {
...
const isBuildIdInProgress = async (buildId) => {
  ...
  try {
    ...
    if (output.builds && output.builds.length === 1) {
      ...
      core.warning(
        `Unexpected amount of builds returned: ${(_a = output.builds) === null || _a === void 0 ? void 0 : _a.length}`,
      );
    }
  } catch (error) {
    const err = error;
  ...
  try {
    ...
    if (output.builds !== undefined) {
      ...
      if (build.logs && build.logs.groupName && build.logs.streamName) {
        ...
        const forwarder = new cloudwatch_logs_forwarder_1.CloudWatchLogsForwarder(
...
const getInProgressProjectToBatchIds = async (codebuildProjects) => {
  const projectsToBuildBatches = await Promise.all(
    codebuildProjects.map(async (codebuildProject) => [
      codebuildProject,
      await (0, exports.getInProgressBuildId)(codebuildProject),
    ]),
  );
